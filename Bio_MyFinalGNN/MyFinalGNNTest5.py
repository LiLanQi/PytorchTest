...
import argparse
import os
from MyFinalGNN5 import MyNewGNN, MyValModel, MyLodaFourGNN
...
writer = SummaryWriter('MyFinalGNNTest05')
...
def load_test_data(phase, label_index, solvent_test, flag):
    data = test_trainset(phase, label_index, solvent_test)
    if(flag == 1):
        loader = torch.utils.data.DataLoader(data, batch_size=args.batchsize, shuffle=True, drop_last=True)
    else:
        loader = torch.utils.data.DataLoader(data, batch_size=args.batchsize, shuffle=False, drop_last=True)
    return loader
...
def val_another_solvent(test_model, test_loader, solute, solvent_test, PATH1, bestSUM_solvent_in_5, bestSUM_solvent_in_10, best_PATH5,
         best_PATH10, current_iter, zero_accuracy, one_accuracy, two_accuracy, three_accuracy, current_epoch, batch_num):
    test_model.eval()
    SUMtest1 = 0
    SUMtest2 = 0
    SUMtest3 = 0
    ...
    if (SUMtest10 > bestSUM_solvent_in_10):
        ...
        best_PATH10 = PATH1 + "-MyFinalGNNTest5_net_in_10.pth.tar"
    if (SUMtest5 > bestSUM_solvent_in_5):
        bestSUM_solvent_in_5 = SUMtest5
        best_PATH5 = PATH1 + "-MyFinalGNNTest5_net_in_5.pth.tar"
...
def train(net, optimizer, criterion, test_model, train_loader, solute, zero_solvent, one_solvent, two_solvent,
          ...
          bestSUM_solvent_in_10, current_iter, current_epoch, batch_num):
    loss_list = []
    # tf_flag = 0
    net.train()
    total_loss = 0
    if(current_iter==0):
        solute_data_four_zerobatch, solvent_data_four_zerobatch, lable_four_zerobatch, solute_data_four_onebatch, solvent_data_four_onebatch, lable_four_onebatch, solute_data_four_twobatch, solvent_data_four_twobatch, lable_four_twobatch, solute_data_four_threebatch, solvent_data_four_threebatch, lable_four_threebatch = get_four_data()
        ...
        with torch.set_grad_enabled(True):  #
            if(current_iter==0):
                if(i_batch>3):
                    ...
                    if(i_batch==0):
                        temp_net = MyLodaFourGNN(nfeat=args.nfeat, nhid=args.nhid, nclass=args.nclass, dropout=args.dropout,
                                       DEVICE=DEVICE,batch_size=args.batchsize)
                        temp_net = temp_net.to(DEVICE)
                        temp_net.load_state_dict(net.state_dict())
                        labels = torch.cat((labels, lable_four_zerobatch), 0)
                        outputs = temp_net(solute_data_zero, solvent_data_zero, solute, zero_solvent,
                                      solute_data_one, solvent_data_one, one_solvent,
                                      solute_data_two, solvent_data_two, two_solvent,
                                      solute_data_three, solvent_data_three, three_solvent,
                                      solute_data_four_zerobatch, solvent_data_four_zerobatch, solvent_test).squeeze(-1)
                    elif(i_batch==1):
                        labels = torch.cat((labels, lable_four_onebatch), 0)
                        outputs = temp_net(solute_data_zero, solvent_data_zero, solute, zero_solvent,
                                      solute_data_one, solvent_data_one, one_solvent,
                                      solute_data_two, solvent_data_two, two_solvent,
                                      solute_data_three, solvent_data_three, three_solvent,
                                      solute_data_four_onebatch, solvent_data_four_onebatch, solvent_test).squeeze(-1)
                    elif (i_batch==2):
                        labels = torch.cat((labels, lable_four_twobatch), 0)
                        outputs = temp_net(solute_data_zero, solvent_data_zero, solute, zero_solvent,
                                      solute_data_one, solvent_data_one, one_solvent,
                                      solute_data_two, solvent_data_two, two_solvent,
                                      solute_data_three, solvent_data_three, three_solvent,
                                      solute_data_four_twobatch, solvent_data_four_twobatch, solvent_test).squeeze(-1)
                    elif (i_batch==3):
                        labels = torch.cat((labels, lable_four_threebatch), 0)
                        outputs = temp_net(solute_data_zero, solvent_data_zero, solute, zero_solvent,
                                      solute_data_one, solvent_data_one, one_solvent,
                                      solute_data_two, solvent_data_two, two_solvent,
                                      solute_data_three, solvent_data_three, three_solvent,
                                      solute_data_four_threebatch, solvent_data_four_threebatch, solvent_test).squeeze(-1)
            else:
                outputs = net(solute_data_zero, solvent_data_zero, solute, zero_solvent,
                              solute_data_one, solvent_data_one, one_solvent,
                              solute_data_two, solvent_data_two, two_solvent,
    ...
    PATH1 = "./check_point/MyFinalGNNTest5/" + str(current_epoch) + "/" +str(current_iter)
    ...
    if(current_iter % 20 ==0):
        ...
        torch.save({'state_dict': net.state_dict(), 'optimizer': optimizer.state_dict()},
               "./check_point/MyFinalGNNTest5/" + str(current_epoch) + "/" + str(current_iter) + "-MyFinalGNNTest5_net.pth.tar")
...
def LOAD(train_lable_index, val_lable_index, test_list, zero_solvent, one_solvent, two_solvent, three_solvent,
    ...
    val_loader = load_data(zero_solvent, "val", one_solvent, val_lable_index, two_solvent,
                           three_solvent)  #
    test_loader = load_test_data("test", test_list, solvent_test, 0)  #
...
def get_four_data():
    ...
    for i in range(0, 128):
        four_data_list.append(i)
    four_loader = load_test_data("test", four_data_list, solvent_test, 0)
    solute_data_four_zerobatch = torch.zeros(3,3)
    solvent_data_four_zerobatch = torch.zeros(3,3)
    lable_four_zerobatch = torch.zeros(3,3)
    solute_data_four_onebatch = torch.zeros(3,3)
    solvent_data_four_onebatch = torch.zeros(3,3)
    lable_four_onebatch = torch.zeros(3,3)
    solute_data_four_twobatch = torch.zeros(3,3)
    solvent_data_four_twobatch = torch.zeros(3,3)
    lable_four_twobatch = torch.zeros(3,3)
    solute_data_four_threebatch = torch.zeros(3,3)
    solvent_data_four_threebatch = torch.zeros(3,3)
    lable_four_threebatch = torch.zeros(3,3)

    for i_batch, batch_data in tqdm(enumerate(four_loader)):
        solute_data_test, solvent_data_test, label_test = batch_data
        labels = label_test.to(DEVICE)
        solute_data_test = solute_data_test.to(DEVICE)
        solvent_data_test = solvent_data_test.to(DEVICE)
        if(i_batch == 0):
            solute_data_four_zerobatch = solute_data_test
            solvent_data_four_zerobatch = solvent_data_test
            lable_four_zerobatch = labels
        if(i_batch == 1):
            solute_data_four_onebatch = solute_data_test
            solvent_data_four_onebatch = solvent_data_test
            lable_four_onebatch = labels
        if (i_batch == 2):
            solute_data_four_twobatch = solute_data_test
            solvent_data_four_twobatch = solvent_data_test
            lable_four_twobatch = labels
        if (i_batch == 3):
            solute_data_four_threebatch = solute_data_test
            solvent_data_four_threebatch = solvent_data_test
            lable_four_threebatch = labels
    return solute_data_four_zerobatch.to(DEVICE),solvent_data_four_zerobatch.to(DEVICE),lable_four_zerobatch.to(DEVICE),\
           solute_data_four_onebatch.to(DEVICE),solvent_data_four_onebatch.to(DEVICE),lable_four_onebatch.to(DEVICE),\
           solute_data_four_twobatch.to(DEVICE),solvent_data_four_twobatch.to(DEVICE),lable_four_twobatch.to(DEVICE),\
           solute_data_four_threebatch.to(DEVICE),solvent_data_four_threebatch.to(DEVICE),lable_four_threebatch.to(DEVICE)
if __name__ == '__main__':
    ...
    print("--------------------------build_model----------------------------")
    net, optimizer, criterion, test_model = build_model(checkpoint_PATH=None)
    ...
    for current_epoch in range(args.epoch):
        root_path = "./check_point/MyFinalGNNTest5/" + str(current_epoch)
