...
from test import get_additional_feature
from MyLastGCN2 import MyNewGCN
...
writer = SummaryWriter('MyLastGCNTest2')
...
class trainset(Dataset):
    def __init__(self, rootdir_ACE, result_src_ACE, phase, rootdir_NMF, result_src_NMF, lable_index, rootdir_meth, solute_addtional_feature, ACE_atom_to_feature,NMF_atom_to_feature,meth_atom_to_feature):
        ...
        for path in path_list_ACE:
            ...
            for i in range(5, len(file_list) - 2):
                ...
                if (i < 173 * 12 + 5):
                    ...
                    temp_result_list.append(ACE_atom_to_feature[temp_atom])
        ...
        for path in path_list_NMF:
            ...
            for i in range(5, len(file_list) - 2):
                ...
                if (i < 173 * 12 + 5):
                    ...
                    temp_result_list.append(NMF_atom_to_feature[temp_atom])
                position_list.append(x)
                position_list.append(y)
                position_list.append(z)
        ...
        lables_list_meth = []
        current_data = 0
        for path in path_list_meth:
            current_data = current_data + 1
            if (current_data % 100 == 9):
                print("path=", path, "current_data=", current_data, ",all_data=", len(path_list_meth), "")
            file = open(path, "r")
            file_list = file.readlines()
            temp_result_list = []
            position_list = []
            for i in range(5, len(file_list) - 2):
                ...
                if (i < 173 * 12 + 5):
                    ...
                    temp_result_list.append(meth_atom_to_feature[temp_atom])
                position_list.append(x)
                position_list.append(y)
                position_list.append(z)
            ...
            solute_data = temp[:173 * 12, :]
            solvent_data = temp[173 * 12:, :]
            solute_list_meth.append(solute_data)  # 存入每个meth文件的溶质图
            solvent_list_meth.append(solvent_data)  # 存入每个meth文件的溶剂图
        for i in range(len(lable_index)):
            lables_list_meth.append(total_labels_meth[lable_index[i]])  # 存入每个meth文件的标签
        self.solute_list_meth = solute_list_meth
        self.solvent_list_meth = solvent_list_meth
        self.lables_list_meth = lables_list_meth # 存入所有mdx 文件对应的output值
    ...
    def __getitem__(self, index):
        ...
        return solute_list_ACE, solvent_list_ACE, lable_ACE, solute_list_NMF, solvent_list_NMF, lable_NMF, solute_list_meth, solvent_list_meth, lable_meth

    def __len__(self):
        return len(self.solute_list_ACE)


def load_train(root_path_ACE, batch_size, result_src_ACE, phase,root_path_NMF,result_src_NMF, label_index, root_path_meth, solute_addtional_feature, ACE_atom_to_feature,NMF_atom_to_feature,meth_atom_to_feature):
    data = trainset(root_path_ACE, result_src_ACE, phase, root_path_NMF, result_src_NMF, label_index, root_path_meth, solute_addtional_feature, ACE_atom_to_feature,NMF_atom_to_feature,meth_atom_to_feature)
    print("data=", data)
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        data_list.append(i)
    train_lable_index, test_lable_index = train_test_split(data_list, train_size=0.72, random_state=42)
    ...
    solute_addtional_feature,ACE_atom_to_feature,NMF_atom_to_feature,wat_atom_to_feature, meth_atom_to_feature = get_additional_feature()
    train_loader = load_train(args.data_src_ACE, args.batchsize, args.result_src_ACE, "train",args.data_src_NMF, args.result_src_NMF, train_lable_index, args.data_src_meth, solute_addtional_feature, ACE_atom_to_feature,NMF_atom_to_feature,meth_atom_to_feature)
    net = MyNewGCN(nfeat=45,nhid=128,nclass=128,dropout=0)
    net = net.to(DEVICE)
    net.train()
    ...
    # ACE_another_feature = ACE_another_feature.to(DEVICE)
    # NMF_another_feature = NMF_another_feature.to(DEVICE)
    # wat_another_feature = wat_another_feature.to(DEVICE)
    for epoch in range(50):
        ...
        for i_batch, batch_data in tqdm(enumerate(train_loader)):
            count = count + 1
            solute_data_ACE, solvent_data_ACE, labels_ACE, solute_data_NMF, solvent_data_NMF, labels_NMF, solute_data_meth, solvent_data_meth, labels_meth = batch_data
            labels_ACE = labels_ACE.to(DEVICE)
            labels_NMF = labels_NMF.to(DEVICE)
            labels_meth = labels_meth.to(DEVICE)
            labels = torch.cat((labels_ACE, labels_NMF), 0)
            labels = torch.cat((labels, labels_meth), 0)
            ...
            with torch.set_grad_enabled(True):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
                outputs = net(solute_data_ACE, solvent_data_ACE, solute_adj, solvent_adj_ACE, solute_data_NMF, solvent_data_NMF, solvent_adj_NMF,
                              solute_data_meth, solvent_data_meth, solvent_adj_meth).squeeze(-1)
    ...
    print('MyLastGCNTest2 Finished Training, lr=', args.lr, "batchsize=", args.batchsize)
    PATH1 = './MyLastGCNTest2_net.pth'
    ...
    test_loader = load_train(args.data_src_ACE, args.batchsize, args.result_src_ACE, "test", args.data_src_NMF,
                              args.result_src_NMF, test_lable_index, args.data_src_meth, solute_addtional_feature,
                              ACE_atom_to_feature, NMF_atom_to_feature, meth_atom_to_feature)
    SUMACE1 = 0
    SUMACE2 = 0
    SUMACE3 = 0
    SUMACE4 = 0
    ...
    for i_batch, batch_data in tqdm(enumerate(test_loader)):
        ...
        with torch.set_grad_enabled(False):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
            outputs = net(solute_data_ACE, solvent_data_ACE, solute_adj, solvent_adj_ACE, solute_data_NMF,
                          solvent_data_NMF, solvent_adj_NMF,solute_data_meth,solvent_data_meth, solvent_adj_meth).squeeze(-1)
    ...
    print("SUMmeth1 = ", SUMmeth1, "SUMmeth2 = ", SUMmeth2, "SUMmeth3 = ", SUMmeth3, "SUMmeth4 = ", SUMmeth4, "SUMmeth5 = ", SUMmeth5)
    print("total", SUMACE5, "ACE in 5%;", "total", SUMNMF5, "NMF in 5%", "total", SUMmeth5, "meth in 5%")
