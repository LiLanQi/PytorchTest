...
from MyLastGCN4 import MyNewGCN
import scipy.sparse as sp
import torch
from tqdm import tqdm
from sklearn.model_selection import train_test_split

# 用ACE+NMF+Water进行模型训练
...
writer = SummaryWriter('MyLastGCNTest4')
...
def train(train_loader, solute_adj, solvent_adj_ACE, solvent_adj_NMF, solvent_adj_wat):
    net.train()
    ...
    for epoch in range(args.epoch):
        ...
        PATH1 = "./check_point/" + str(epoch) + 'MyLastGCNTest4_net.pth'
        torch.save(net.state_dict(), PATH1)
    print("loss_list = ", loss_list)
    print('MyLastGCNTest4 Finished Training, lr=', args.lr, "batchsize=", args.batchsize)
...
if __name__ == '__main__':
    ...
    net = MyNewGCN(nfeat=45, nhid=64, nclass=128, dropout=0, DEVICE=DEVICE)
    net = net.to(DEVICE)
    train(train_loader, solute_adj, solvent_adj_ACE, solvent_adj_NMF, solvent_adj_wat)

    # test
    net.eval()
    test_loader = load_data(args.data_src_ACE, args.batchsize, "test", args.data_src_NMF, test_lable_index, args.data_src_wat)
    test(test_loader, test_lable_index, solute_adj, solvent_adj_ACE, solvent_adj_NMF, solvent_adj_wat)
