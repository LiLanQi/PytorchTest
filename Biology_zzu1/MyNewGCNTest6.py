...
parser.add_argument('--data_index', type=int, default=1000, help='data_index')

DEVICE = torch.device('cuda:0')
args = parser.parse_args()
writer = SummaryWriter('MynewGCNTest6')
...
class trainset(Dataset):
    ...
    def __init__(self,ACE_solvent, ACE_root_path, lable_index, solute_edge_features,
    ACE_solvent_edge_features, solute_edge_u, solute_edge_v, ACE_solvent_edge_u, ACE_solvent_edge_v,
    NMF_solvent, NMF_root_path, NMF_solvent_edge_features, NMF_solvent_edge_u, NMF_solvent_edge_v):
        solute_list = []
        solvent_list = []
        lables_list = []
        ACE_path_list= readFileData(ACE_solvent, ACE_root_path, lable_index)  # 得到对应ACEx.pdb文件的list
        NMF_path_list = readFileData(NMF_solvent, NMF_root_path, lable_index)  # 得到对应ACEx.pdb文件的list
        # 对所有的ACE.pdb文件里的数据进行预处理
        current_data = 0
        atoms_dict = {'H': [1,0,0,0,0,0], 'C': [0,1,0,0,0,0], 'O': [0,0,1,0,0,0], 'N': [0,0,0,1,0,0], 'S': [0,0,0,0,1,0],
                'F': [0,0,0,0,0,1]}  # count0= 4566 countN= 3486 countC= 13194 countF= 216 countS= 324 countH= 26274
        for path in ACE_path_list:
            solute = dgl.DGLGraph()
            solvent = dgl.DGLGraph()
            current_data = current_data + 1
            if(current_data%100 == 9):
                print("path=",path,"current_data=",current_data,",all_data=",len(ACE_path_list),"")
            file = open(path, "r")
            file_list = file.readlines()
            temp_result_list = []
            ...
            temp = torch.tensor(temp_result_list).reshape(-1, 9)
        ...
        for path in NMF_path_list:
            ...
            solute.add_edge(solute_edge_u,solute_edge_v)
            solute.edata['w'] = solute_edge_features
            solute_list.append(solute)  # 存入每个NMF文件的溶质图
            solvent_data = temp[173 * 12:, :]
            solvent.add_nodes(9 * 1350) #NMF
            solvent.ndata['x'] = solvent_data
            solvent.add_edge(NMF_solvent_edge_u, NMF_solvent_edge_v)
            solvent.edata['w'] = NMF_solvent_edge_features
            solvent_list.append(solvent)  # 存入每个文件的溶剂图

        for i in range(len(lable_index)):
            lables_list.append(ACE_total_labels[lable_index[i]])# 存入每个ACE文件的标签
        for i in range(len(lable_index)):
            lables_list.append(NMF_total_labels[lable_index[i]])# 存入每个NMF文件的标签
...
def load_train(ACE_solvent, collate, ACE_root_path, batch_size, label_index,solute_edge_features,
               ACE_solvent_edge_features,solute_edge_u, solute_edge_v,ACE_solvent_edge_u, ACE_solvent_edge_v,
               NMF_solvent, NMF_root_path, NMF_solvent_edge_features,  NMF_solvent_edge_u, NMF_solvent_edge_v):
    # get ACE and NMF data
    data = trainset(ACE_solvent, ACE_root_path, label_index,solute_edge_features,
                    ACE_solvent_edge_features,solute_edge_u, solute_edge_v, ACE_solvent_edge_u, ACE_solvent_edge_v,
                    NMF_solvent, NMF_root_path, NMF_solvent_edge_features,  NMF_solvent_edge_u, NMF_solvent_edge_v
    )
    print("data=", data)
...
def train(model, epoch, train_loader, val_loader, criterion):
    model.train()
    train_loss = []
    running_loss = 0
    count = 0
...
if __name__ == '__main__':
    ...
    train_loader_ACE_NMF = load_train("ACE", collate, args.ACE_data_src, args.batchsize,
                                  train_lable_index, solute_edge_features, ACE_solvent_edge_features,
                                  solute_edge_u, solute_edge_v, ACE_solvent_edge_u, ACE_solvent_edge_v,
                                  "NMF", args.NMF_data_src, NMF_solvent_edge_features, NMF_solvent_edge_u, NMF_solvent_edge_v)
    ...
    for epoch in range(10):
        ...
        train(model, epoch, train_loader_ACE_NMF, "val_loader_NMF", criterion)
    ...
    test_loader_ACE_NMF = load_train("ACE", collate, args.ACE_data_src, args.batchsize,
                                  test_lable_index, solute_edge_features, ACE_solvent_edge_features,
                                  solute_edge_u, solute_edge_v, ACE_solvent_edge_u, ACE_solvent_edge_v,
                                  "NMF", args.NMF_data_src, NMF_solvent_edge_features, NMF_solvent_edge_u, NMF_solvent_edge_v)

    # test(test_loader_ACE, "ACE")
    test(test_loader_ACE_NMF, "ACENMF")
