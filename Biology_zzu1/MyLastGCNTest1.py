...
from MyLastGCN1 import MyNewGCN
...
writer = SummaryWriter('MyLastGCNTest1')
...
class trainset(Dataset):
    def __init__(self, rootdir_ACE, result_src_ACE, phase, rootdir_NMF, result_src_NMF, lable_index, rootdir_wat, solute_addtional_feature, ACE_atom_to_feature,NMF_atom_to_feature,wat_atom_to_feature):
...
def load_train(root_path_ACE, batch_size, result_src_ACE, phase,root_path_NMF,result_src_NMF, label_index, root_path_wat, solute_addtional_feature, ACE_atom_to_feature,NMF_atom_to_feature,wat_atom_to_feature):
    data = trainset(root_path_ACE, result_src_ACE, phase, root_path_NMF, result_src_NMF, label_index, root_path_wat, solute_addtional_feature, ACE_atom_to_feature,NMF_atom_to_feature,wat_atom_to_feature)
    print("data=", data)
...
if __name__ == '__main__':
    ...
    train_loader = load_train(args.data_src_ACE, args.batchsize, args.result_src_ACE, "train",args.data_src_NMF, args.result_src_NMF, train_lable_index, args.data_src_wat, solute_addtional_feature, ACE_atom_to_feature,NMF_atom_to_feature,wat_atom_to_feature)
        ...
        for i_batch, batch_data in tqdm(enumerate(train_loader)):
            ...
            with torch.set_grad_enabled(True):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
                outputs = net(solute_data_ACE, solvent_data_ACE, solute_adj, solvent_adj_ACE, solute_data_NMF, solvent_data_NMF, solvent_adj_NMF,
                              solute_data_wat, solvent_data_wat, solvent_adj_wat).squeeze(-1)
    ...
    print('MyLastGCNTest1 Finished Training, lr=', args.lr, "batchsize=", args.batchsize)
    PATH1 = './MyLastGCNTest1_net.pth'
    ...
    test_loader = load_train(args.data_src_ACE, args.batchsize, args.result_src_ACE, "test", args.data_src_NMF,
                              args.result_src_NMF, test_lable_index, args.data_src_wat, solute_addtional_feature,
                              ACE_atom_to_feature, NMF_atom_to_feature, wat_atom_to_feature)
    SUMACE1 = 0
    SUMACE2 = 0
    SUMACE3 = 0
    SUMACE4 = 0
    ...
    print("total", SUMACE5, "ACE in 5%;", "total", SUMNMF5, "NMF in 5%", "total", SUMWAT5, "WAT in 5%")
