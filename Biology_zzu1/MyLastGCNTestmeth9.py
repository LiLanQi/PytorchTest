...
class MyValModel(nn.Module):
    ...
    def get_graph_pool_solute_and_meth(self):
        temp_solute0 = torch.ones(1, 18411)
        temp_solute1 = torch.zeros(1, 18411)
        temp_solute2 = torch.cat((temp_solute1, temp_solute1), 1)
        temp_solute3 = torch.cat((temp_solute2, temp_solute1), 1)
        graph_pool_solute0 = torch.cat((temp_solute0, temp_solute3), 1)
        graph_pool_solute1 = torch.cat((torch.cat((temp_solute1, temp_solute0), 1), temp_solute2), 1)
    ...
    def __init__(self, nfeat, nhid, nclass, dropout, DEVICE):
        ...
        self.fc2 = nn.Linear(3 * nclass, nclass)
        ...
        self.graph_pool_solute_and_meth = self.get_graph_pool_solute_and_meth()
        self.DEVICE = DEVICE
    ...
    def forward(self, solute_adj, solute_meth, solvent_meth, solvent_adj_meth, solute_and_meth_adj):
        ...
        graph_pool_solute_and_meth = self.graph_pool_solute_and_meth.to(self.DEVICE)

        solute_and_meth_feature = torch.cat((solute_meth, solvent_meth), 1)
        ...
        init_solute_and_meth_feature = self.fc1(solute_and_meth_feature)
        solute_and_meth_feature = F.relu(self.gc1(solute_and_meth_feature, solute_and_meth_adj))
        solute_and_meth_feature = self.gc2(solute_and_meth_feature, solute_and_meth_adj) + init_solute_and_meth_feature



        solute_meth = solute_meth.reshape(-1, self.nclass)
        solvent_meth = solvent_meth.reshape(-1, self.nclass)
        solute_and_meth_feature = solute_and_meth_feature.reshape(-1, self.nclass)
        ...
        solute_and_meth_feature = torch.mm(graph_pool_solute_and_meth, solute_and_meth_feature)

        data2 = torch.cat((solute_meth, solvent_meth), 1)
        data2 = torch.cat((data2, solute_and_meth_feature), 1)
