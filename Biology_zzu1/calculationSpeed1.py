...
class trainset(Dataset):
    ...
    def __init__(self, rootdir_ACE, phase, rootdir_NMF,  lable_index, rootdir_wat, rootdir_meth):
        ...
        solute_speed_list_ACE = {}
        solvent_speed_list_ACE = {}
        solute_solvent_speed_list_ACE = {}
        solute_speed_list_ACE[0] = torch.tensor(0)
        solvent_speed_list_ACE[0] = torch.tensor(0)
        solute_solvent_speed_list_ACE[0] = torch.tensor(0)
        for i in range(1000):
            if(i!=0):
                before_solute_ACE = self.solute_list_ACE[i-1]
                cur_solute_ACE = self.solute_list_ACE[i]
                speed = cur_solute_ACE - before_solute_ACE
                speed=torch.norm(speed, p=2, dim=1)
                len = speed.shape[0]
                speed = torch.sum(speed) / len
                solute_speed_list_ACE[i] = speed


                before_solvent_ACE = self.solvent_list_ACE[i-1]
                cur_solvent_ACE = self.solvent_list_ACE[i]
                speed = cur_solvent_ACE - before_solvent_ACE
                speed = torch.norm(speed, p=2, dim=1)
                len = speed.shape[0]
                speed = torch.sum(speed) / len
                solvent_speed_list_ACE[i] = speed

                solute_solvent_speed_list_ACE[i] = (solute_speed_list_ACE[i] + solvent_speed_list_ACE[i])/2
        np.save('./solute_speed_list_ACE_for_all_system.npy', solute_speed_list_ACE)
        np.save('./solvent_speed_list_ACE_for_all_system.npy', solvent_speed_list_ACE)
        np.save('./solute_solvent_speed_list_ACE_for_all_system.npy', solute_solvent_speed_list_ACE)

        self.solute_list_NMF, self.solvent_list_NMF, self.lables_list_NMF = self.__read_data__(path_list_NMF,
                                                                                               lable_index, "NMF")

        solute_speed_list_NMF = {}
        solvent_speed_list_NMF = {}
        solute_solvent_speed_list_NMF = {}
        solute_speed_list_NMF[0] = torch.tensor(0)
        solvent_speed_list_NMF[0] = torch.tensor(0)
        solute_solvent_speed_list_NMF[0] = torch.tensor(0)
        for i in range(1000):
            if (i != 0):
                before_solute_NMF = self.solute_list_NMF[i - 1]
                cur_solute_NMF = self.solute_list_NMF[i]
                speed = cur_solute_NMF - before_solute_NMF
                speed = torch.norm(speed, p=2, dim=1)
                len = speed.shape[0]
                speed = torch.sum(speed)/len
                solute_speed_list_NMF[i] = speed

                before_solvent_NMF = self.solvent_list_NMF[i - 1]
                cur_solvent_NMF = self.solvent_list_NMF[i]
                speed = cur_solvent_NMF - before_solvent_NMF
                speed = torch.norm(speed, p=2, dim=1)
                len = speed.shape[0]
                speed = torch.sum(speed)/len
                solvent_speed_list_NMF[i] = speed

                solute_solvent_speed_list_NMF[i] = (solute_speed_list_NMF[i] + solvent_speed_list_NMF[i]) / 2
        np.save('./solute_speed_list_NMF_for_all_system.npy', solute_speed_list_NMF)
        np.save('./solvent_speed_list_NMF_for_all_system.npy', solvent_speed_list_NMF)
        np.save('./solute_solvent_speed_list_NMF_for_all_system.npy', solute_solvent_speed_list_NMF)

        self.solute_list_wat, self.solvent_list_wat, self.lables_list_wat = self.__read_data__(path_list_wat,
                                                                                               lable_index, "wat")


        solute_speed_list_wat = {}
        solvent_speed_list_wat = {}
        solute_solvent_speed_list_wat = {}
        solute_speed_list_wat[0] = torch.tensor(0)
        solvent_speed_list_wat[0] = torch.tensor(0)
        solute_solvent_speed_list_wat[0] = torch.tensor(0)
        for i in range(1000):
            if (i != 0):
                before_solute_wat = self.solute_list_wat[i - 1]
                cur_solute_wat = self.solute_list_wat[i]
                speed = cur_solute_wat - before_solute_wat
                speed = torch.norm(speed, p=2, dim=1)
                len = speed.shape[0]
                speed = torch.sum(speed) / len
                solute_speed_list_wat[i] = speed

                before_solvent_wat = self.solvent_list_wat[i - 1]
                cur_solvent_wat = self.solvent_list_wat[i]
                speed = cur_solvent_wat - before_solvent_wat
                speed = torch.norm(speed, p=2, dim=1)
                len = speed.shape[0]
                speed = torch.sum(speed) / len
                solvent_speed_list_wat[i] = speed

                solute_solvent_speed_list_wat[i] = (solute_speed_list_wat[i] + solvent_speed_list_wat[i]) / 2
        np.save('./solute_speed_list_wat_for_all_system.npy', solute_speed_list_wat)
        np.save('./solvent_speed_list_wat_for_all_system.npy', solvent_speed_list_wat)
        np.save('./solute_solvent_speed_list_wat_for_all_system.npy', solute_solvent_speed_list_wat)

        self.solute_list_meth, self.solvent_list_meth, self.lables_list_meth = self.__read_data__(path_list_meth,
                                                                                                  lable_index, "meth")
        solute_speed_list_meth = {}
        solvent_speed_list_meth = {}
        solute_solvent_speed_list_meth = {}
        solute_speed_list_meth[0] = torch.tensor(0)
        solvent_speed_list_meth[0] = torch.tensor(0)
        solute_solvent_speed_list_meth[0] = torch.tensor(0)
        for i in range(1000):
            if (i != 0):
                before_solute_meth = self.solute_list_meth[i - 1]
                cur_solute_meth = self.solute_list_meth[i]
                speed = cur_solute_meth - before_solute_meth
                speed = torch.norm(speed, p=2, dim=1)
                len = speed.shape[0]
                speed = torch.sum(speed) / len
                solute_speed_list_meth[i] = speed

                before_solvent_meth = self.solvent_list_meth[i - 1]
                cur_solvent_meth = self.solvent_list_meth[i]
                speed = cur_solvent_meth - before_solvent_meth
                speed = torch.norm(speed, p=2, dim=1)
                len = speed.shape[0]
                speed = torch.sum(speed) / len
                solvent_speed_list_meth[i] = speed

                solute_solvent_speed_list_meth[i] = (solute_speed_list_meth[i] + solvent_speed_list_meth[i]) / 2
        np.save('./solute_speed_list_meth_for_all_system.npy', solute_speed_list_meth)
        np.save('./solvent_speed_list_meth_for_all_system.npy', solvent_speed_list_meth)
        np.save('./solute_solvent_speed_list_meth_for_all_system.npy', solute_solvent_speed_list_meth)
    ...
    def __read_data__(self, path_list, lable_index, solvent):
        ...
        for path in path_list:
            ...
            position_tensor = torch.tensor(position_list).reshape(-1, 3)
            solute_data = position_tensor[:173 * 12, :]
            solvent_data = position_tensor[173 * 12:, :]
            solute_list.append(solute_data)  # 存入每个ACE文件的溶质图
            solvent_list.append(solvent_data)  # 存入每个ACE文件的溶剂图
        for i in range(len(lable_index)):
            lables_list.append(total_labels[lable_index[i]])  # 存入每个ACE文件的标签
    ...
    def __getitem__(self, index):
        ...
        return solute_list_ACE, solvent_list_ACE, lable_ACE

    def __len__(self):
        return len(self.solute_list_ACE)


def load_data(root_path_ACE, batch_size, phase, root_path_NMF, label_index,root_path_wat, root_path_meth):
    data = trainset(root_path_ACE, phase, root_path_NMF,label_index, root_path_wat, root_path_meth)
    print("data=", data)
...
def train(net, optimizer, criterion, test_model,train_loader, solute_adj, solvent_adj_ACE, solvent_adj_NMF, solvent_adj_wat, val_loader, solvent_adj_meth, test_loader):
    best_PATH5 = ""
    best_PATH10 = ""
    bestSUMmeth5 = 0
    bestSUMmeth10 = 0
...
if __name__ == '__main__':
    ...
    def LOAD_TRAIN(train_lable_index):
        train_loader = load_data(args.data_src_ACE, args.batchsize, "train", args.data_src_NMF, train_lable_index, args.data_src_wat, args.data_src_meth)#加载训练用的ACE+NMF+Water
        ...
        # train(net, optimizer, criterion, test_model, train_loader, solute_adj, solvent_adj_ACE, solvent_adj_NMF, solvent_adj_wat, val_loader, solvent_adj_meth, test_loader)


    for i in range(0, 1000):
        data_list.append(i)
    temp_train_lable_index = data_list
    # temp_val_lable_index = data_list[i * 28: (i + 1) * 28]
    # test_list = data_list[i*100:(i+1)*100]
    LOAD_TRAIN(temp_train_lable_index)
