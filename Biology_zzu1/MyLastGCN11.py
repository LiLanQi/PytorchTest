...
class MyNewGCN(nn.Module):
    ...
    def get_graph_pool(self, solvent):
        ...
        print("solvent=", solvent, "graph_pool_solute.shape=", graph_pool_solute.shape, "graph_pool_solvent.shape=",
              graph_pool_solvent.shape)
    ...
    def forward(self, solute_ACE, solvent_ACE, solute_adj, solvent_adj_ACE, solute_NMF, solvent_NMF, solvent_adj_NMF,
        ...
        init_solute_DMF = self.fc1(solute_DMF).reshape(-1, self.nclass)
        ...
        solute_DMF = solute_DMF.reshape(solute_DMF.shape[0] * solute_DMF.shape[1], -1)
        solute_DMF = F.relu(self.conv1(solute_DMF, solute_adj))
        solute_DMF = self.conv2(solute_DMF, solute_adj) + init_solute_DMF
        ...
        init_solvent_DMF = self.fc1(solvent_DMF).reshape(-1, self.nclass)
        # 溶剂进行3层GCN+resnet
        ...
        solvent_DMF = solvent_DMF.reshape(solvent_DMF.shape[0] * solvent_DMF.shape[1], -1)
        solvent_DMF = F.relu(self.conv1(solvent_DMF, solvent_adj_DMF))
        solvent_DMF = self.conv2(solvent_DMF, solvent_adj_DMF) + init_solvent_DMF
        # solvent_wat("solvent.shape=", solvent.shape)#(batch_size, 8940, 16))
        ...
        solute_DMF = solute_DMF.reshape(self.batch_size, -1, self.nclass)
        solvent_DMF = solvent_DMF.reshape(self.batch_size, -1, self.nclass)
