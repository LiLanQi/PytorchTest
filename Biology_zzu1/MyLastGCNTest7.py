...
import argparse
import os
from MyLastGCNTestmeth7 import MyValModel
from MyLastGCN7 import MyNewGCN
import scipy.sparse as sp
import torch
from tqdm import tqdm
from sklearn.model_selection import train_test_split

# 用ACE+NMF进行模型训练,测试meth
...
writer = SummaryWriter('MyLastGCNTest7')
...
def load_val_data(batch_size, phase, label_index, root_path_meth):
    data = val_trainset(phase, label_index, root_path_meth)
    print("data=", data)
    loader = torch.utils.data.DataLoader(data, batch_size=batch_size, shuffle=True, drop_last=False, num_workers=12)
    return loader

def test(test_loader, test_lable_index, solute_adj, solvent_adj_ACE, solvent_adj_NMF, solvent_adj_wat):
    SUMACE1 = 0
    SUMACE2 = 0
    SUMACE3 = 0
    SUMACE4 = 0
    ...
    for i_batch, batch_data in tqdm(enumerate(test_loader)):
        ...
        with torch.set_grad_enabled(False):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
            outputs = net(solute_data_ACE, solvent_data_ACE, solute_adj, solvent_adj_ACE, solute_data_NMF,
                          solvent_data_NMF, solvent_adj_NMF, solute_data_NMF, solvent_data_wat, solvent_adj_wat).squeeze(-1)
    ...
    print("SUMwat1 = ", SUMwat1, "SUMwat2 = ", SUMwat2, "SUMwat3 = ", SUMwat3, "SUMwat4 = ", SUMwat4,
          "SUMwath5 = ", SUMwat5)
    print("total ", (SUMACE5*100//len(test_lable_index)), "% ACE in 5%;", "total ", (SUMNMF5*100//len(test_lable_index)), "% NMF in 5%;", "total ", (SUMwat5*100//len(test_lable_index)), "% wat in 5%")

def val(val_model, val_loader, solute_adj, solvent_adj_meth, PATH1, bestSUMmeth5, bestSUMmeth10, best_PATH5, best_PATH10):
    val_model.eval()
    SUMmeth1 = 0
    SUMmeth2 = 0
    SUMmeth3 = 0
    ...
    for i_batch, batch_data in tqdm(enumerate(val_loader)):
        ...
        with torch.set_grad_enabled(False):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
            outputs = val_model(solute_adj, solute_data_meth, solvent_data_meth, solvent_adj_meth).squeeze(-1)
        deviation = abs((outputs - labels) / labels)
        print("outputs=", outputs)
        print("labels=", labels)
        print("deviation=", deviation)
    ...
    print("SUMmeth6 = ", SUMmeth6, "SUMmeth7 = ", SUMmeth7, "SUMmeth8 = ", SUMmeth8, "SUMmeth9 = ", SUMmeth9,
          "SUMmeth10 = ", SUMmeth10)
    print("tatol", (SUMmeth5*100)/(len(val_loader)*4), "% meth in 5%", "tatol", (SUMmeth10*100)/(len(val_loader)*4), "% meth in 10%")
    return best_PATH5, best_PATH10, bestSUMmeth5, bestSUMmeth10

def train(train_loader, solute_adj, solvent_adj_ACE, solvent_adj_NMF, solvent_adj_wat, val_loader, solvent_adj_meth):
    net.train()
    ...
    for epoch in range(args.epoch):
        ...
        PATH1 = "./check_point/" + str(epoch) + 'MyLastGCNTest7_net.pth'
        torch.save(net.state_dict(), PATH1)

        val_model = MyValModel(nfeat=9, nhid=64, nclass=128, dropout=0, DEVICE=DEVICE)
        val_model = val_model.to(DEVICE)
        val_model.load_state_dict(torch.load(PATH1))
        best_PATH5, best_PATH10, bestSUMmeth5, bestSUMmeth10 = val(val_model, val_loader, solute_adj, solvent_adj_meth, PATH1, bestSUMmeth5, bestSUMmeth10, best_PATH5, best_PATH10)
        print("best_PATH5=", best_PATH5, "best_PATH10=", best_PATH10)
    print("loss_list = ", loss_list)
    print('MyLastGCNTest7 Finished Training, lr=', args.lr, "batchsize=", args.batchsize)
...
if __name__ == '__main__':
    ...
    train_loader = load_data(args.data_src_ACE, args.batchsize, "train", args.data_src_NMF, train_lable_index, args.data_src_wat)
    test_loader = load_data(args.data_src_ACE, args.batchsize, "train", args.data_src_NMF, test_lable_index,
                             args.data_src_wat)
    val_loader = load_val_data(args.batchsize, "val",data_list, args.data_src_meth)
    net = MyNewGCN(nfeat=9, nhid=64, nclass=128, dropout=0.5, DEVICE=DEVICE)
    net = net.to(DEVICE)
    train(train_loader, solute_adj, solvent_adj_ACE, solvent_adj_NMF, solvent_adj_wat, val_loader, solvent_adj_meth)
