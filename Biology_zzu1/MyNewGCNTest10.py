...
import argparse
import os
from MyNewGCN1 import MyNewGCN
...
class trainset(Dataset):
    def __init__(self, rootdir_ACE, result_src_ACE, phase, rootdir_NMF, result_src_NMF, lable_index):
        ...
        solvent_list_ACE = []
        lables_list_ACE = []
        path_list_ACE = readFileData(rootdir_ACE, lable_index)  # 得到对应ACEx.pdb文件的list
        path_list_NMF = readFileData(rootdir_NMF, lable_index)  # 得到对应ACEx.pdb文件的list
...
if __name__ == '__main__':
    ...
    train_loader = load_train(args.data_src_ACE, args.batchsize, args.result_src_ACE, "train",args.data_src_NMF, args.result_src_NMF, train_lable_index)
    ...
    for epoch in range(10):
        ...
        for i_batch, batch_data in tqdm(enumerate(train_loader)):
            ...
            with torch.set_grad_enabled(True):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
                outputs = net(solute_data_ACE, solvent_data_ACE, solute_adj, solvent_adj_ACE, solute_data_NMF, solvent_data_NMF, solvent_adj_NMF).squeeze(-1)
