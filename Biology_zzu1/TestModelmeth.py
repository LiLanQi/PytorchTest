...
import argparse
import os
from MyLastGCNTestmeth import MyNewGCN
...
class trainset(Dataset):
    def __init__(self, rootdir_ACE, result_src_ACE, phase, rootdir_NMF, result_src_NMF, lable_index, rootdir_wat, rootdir_meth, solute_addtional_feature, meth_atom_to_feature):
        self.root = rootdir_ACE
        path_list_meth = readFileData(rootdir_meth, lable_index, "meth")  # 得到对应watx.pdb文件的list
        ...
        for i in range(len(lable_index)):
            lables_list_meth.append(total_labels_meth[lable_index[i]])  # 存入每个wat文件的标签
...
def load_train(root_path_ACE, batch_size, result_src_ACE, phase,root_path_NMF,result_src_NMF, label_index, root_path_wat, root_path_meth, solute_addtional_feature, meth_atom_to_feature):
    data = trainset(root_path_ACE, result_src_ACE, phase, root_path_NMF, result_src_NMF, label_index, root_path_wat, root_path_meth, solute_addtional_feature, meth_atom_to_feature)
    print("data=", data)
...
if __name__ == '__main__':
    ...
    test_loader = load_train(args.data_src_ACE, args.batchsize, args.result_src_ACE, "test", args.data_src_NMF,
                             ...
                             solute_addtional_feature, meth_atom_to_feature)
    SUMmeth1 = 0
    SUMmeth2 = 0
    SUMmeth3 = 0
    SUMmeth4 = 0
    ...
    for i_batch, batch_data in tqdm(enumerate(test_loader)):
        ...
        with torch.set_grad_enabled(False):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
            outputs = net(solute_adj,solute_data_meth,
                          solvent_data_meth, solvent_adj_meth).squeeze(-1)
    ...
    print("SUMmeth6 = ", SUMmeth6, "SUMmeth7 = ", SUMmeth7, "SUMmeth8 = ", SUMmeth8, "SUMmeth9 = ", SUMmeth9, "SUMmeth10 = ", SUMmeth10)
    print("tatol", SUMmeth5, " meth in 5%")
