...
writer = SummaryWriter('MynewGCNTest7')
...
if __name__ == '__main__':
    ...
    for epoch in range(100):
        ...
        for i_batch, batch_data in tqdm(enumerate(train_loader)):
            ...
            with torch.set_grad_enabled(True):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
                outputs = net(solute_data, solvent_data, solute_adj, solvent_adj, batch_data).squeeze(-1)
    ...
    print('MynewGCNTest7 Finished Training, lr=', args.lr, "batchsize=", args.batchsize)
    PATH1 = './MynewGCNTest7_net.pth'
