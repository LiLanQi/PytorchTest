...
def get_solute_adj():
    ...
    for connect_atom in bonds:
        ...
        for i in range(12):
            ...
            solute_adj += [(left_atom_value, right_atom_value), (right_atom_value, left_atom_value)]
    for i in range(2076):
        solute_adj += [(i, i)]
    return solute_adj
...
def get_solvent_adj(solvent, dim):
    ...
    for connect_atom in bonds:
        ...
        for i in range(atom_g):
            ...
            solvent_adj += [(left_atom_value, right_atom_value), (right_atom_value, left_atom_value)]
...
class trainset(Dataset):
    def __init__(self, rootdir_ACE, result_src_ACE, phase, rootdir_NMF, result_src_NMF, lable_index, rootdir_wat):
        ...
        atoms_dict = {'H': 1, 'C': 6, 'O': 8, 'N': 7, 'S': 16,
                      'F': 19}  # count0= 4566 countN= 3486 countC= 13194 countF= 216 countS= 324 countH= 26274
...
def load_train(root_path_ACE, batch_size, result_src_ACE, phase,root_path_NMF,result_src_NMF, label_index, root_path_wat):
    data = trainset(root_path_ACE, result_src_ACE, phase, root_path_NMF, result_src_NMF, label_index, root_path_wat)
    print("data=", data)
...
if __name__ == '__main__':
    ...
    train_loader = load_train(args.data_src_ACE, args.batchsize, args.result_src_ACE, "train",args.data_src_NMF, args.result_src_NMF, train_lable_index, args.data_src_wat)
    ...
    loss_list = []
    solute_enviroment_data, ACE_solvent_enviroment_data, NMF_solvent_enviroment_data, wat_solvent_enviroment_data = get_additional_feature()
    for epoch in range(10):
        ...
        for i_batch, batch_data in tqdm(enumerate(train_loader)):
            ...
            with torch.set_grad_enabled(True):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
                outputs = net(solute_data_ACE, solvent_data_ACE, solute_adj, solvent_adj_ACE, solute_data_NMF, solvent_data_NMF, solvent_adj_NMF, solute_data_wat, solvent_data_wat, solvent_adj_wat, solute_enviroment_data, ACE_solvent_enviroment_data, NMF_solvent_enviroment_data, wat_solvent_enviroment_data).squeeze(-1)
    ...
    test_loader = load_train(args.data_src_ACE, args.batchsize, args.result_src_ACE, "test", args.data_src_NMF,
                              args.result_src_NMF, test_lable_index, args.data_src_wat)
    SUMACE1 = 0
    SUMACE2 = 0
    SUMACE3 = 0
    SUMACE4 = 0
    ...
    for i_batch, batch_data in tqdm(enumerate(test_loader)):
        ...
        with torch.set_grad_enabled(False):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
            outputs = net(solute_data_ACE, solvent_data_ACE, solute_adj, solvent_adj_ACE, solute_data_NMF,
                          ...
                          solute_enviroment_data, ACE_solvent_enviroment_data, NMF_solvent_enviroment_data,
                          wat_solvent_enviroment_data).squeeze(-1)
