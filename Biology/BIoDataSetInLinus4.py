...
class trainset(Dataset):
    def __init__(self, rootdir, result_src, phase):
        ...
        for path in path_list:
            ...
            data = torch.zeros((1, 160, 70))
            for i in range(temp.shape[0]):
                x = int(temp[i][0])
                y = int(temp[i][1])
                atom = chr(int(temp[i][3]))
                if ((x < 160) and (y < 70)):
                    data[0][x][y] += dict[atom]
...
if __name__ == '__main__':
    ...
    print('BIoDataSetInLinus4 Finished Training, lr=',args.lr,"batchsize=",args.batchsize)
    PATH = './bio_net4.pth'
