...
class trainset(Dataset):
    def __init__(self, rootdir, result_src, phase):
        ...
        for path in path_list:
            ...
            data = torch.zeros((1000, 70, 40)) #时间维度100，y=70，z=40
            for i in range(temp.shape[0]):
                y = int(temp[i][1])
                z = int(temp[i][2])
                atom = chr(int(temp[i][3]))
                if ((z < 40) and (y < 70)):
                    data[count][y][z] = max(dict[atom], data[count][y][z])
...
if __name__ == '__main__':
    ...
    print('BIoDataSetInLinus7 Finished Training, lr=',args.lr,"batchsize=",args.batchsize)
    PATH = './bio_net7.pth'
