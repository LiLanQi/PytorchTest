...
class MyNewGNN(nn.Module):
    ...
    def forward(self, solute_data_zero, solvent_data_zero, solute, zero_solvent,
        ...
        solute = torch.cat((solute_zero, solute_one), 0)
        temp = torch.cat((solute_two, solute_three), 0)
        solute = torch.cat((solute, temp), 0)
        solute = torch.cat((solute, solute_four), 0)

        solvent = torch.cat((solvent_zero, solvent_one), 0)
        temp = torch.cat((solvent_two, solvent_three), 0)
        solvent = torch.cat((solvent, temp), 0)
        solvent = torch.cat((solvent, solvent_four), 0)

        solution = torch.cat((solute, solvent), 1)
        ...
        temp = torch.cat((data3, data4), 0)
        data = torch.cat((data, temp), 0)
        data = torch.cat((data, data5), 0)

        return solute, solvent, solution, data
