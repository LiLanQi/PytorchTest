...
from MyFinalGNN0 import MyNewGNN, MyValModel
...
parser.add_argument('--checkpoint_PATH', type=str, default="./check_point/MyFinalGNNTest4_patience100/0/169-MyFinalGNNTest4_net_in_5.pth.tar")
...
# writer = SummaryWriter('Test_solvent2')
...
def val(net, val_loader, solute, zero_solvent, one_solvent, two_solvent,three_solvent):
    ...
    return SUMzero5, SUMone5, SUMtwo5, SUMthree5, len(val_loader) * args.batchsize


def val_another_solvent(test_model, test_loader, solute, solvent_test, true_label_data, pred_label_data, current_iter):
    test_model.eval()
    SUMtest1 = 0
    SUMtest2 = 0
    SUMtest3 = 0
    ...
    for i_batch, batch_data in tqdm(enumerate(test_loader)):
        ...
        deviation = abs((outputs - labels) / labels)
        if (solvent_test["solvent"] == "meth"):
            labels = labels.cpu().detach().numpy().reshape(-1, 1)
            outputs = outputs.cpu().detach().numpy().reshape(-1, 1)
            if (current_iter == 0):
                true_label_data = labels
                pred_label_data = outputs
            else:
                true_label_data = numpy.vstack((true_label_data, labels))
                pred_label_data = numpy.vstack((pred_label_data, outputs))

            np.savetxt('../meth_true_label_data.csv', true_label_data, delimiter=',')
            np.savetxt('../meth_pred_label_data.csv', pred_label_data, delimiter=',')
        for i in range(deviation.shape[0]):
            if (i < args.batchsize):
                if (deviation[i] < 0.01):
                    SUMtest1 = SUMtest1 + 1
    ...
    #     best_PATH10 = PATH1 + "-MyFinalGNNTest3_net_in_10.pth.tar"
    # if (SUMtest5 > bestSUM_solvent_in_5):
    #     bestSUM_solvent_in_5 = SUMtest5
    #     best_PATH5 = PATH1 + "-MyFinalGNNTest3_net_in_5.pth.tar"
    ...
    return SUMtest5, SUMtest10, len(test_loader) * args.batchsize, true_label_data, pred_label_data
...
def LOAD(know_solvent_test_index, unknow_solvent_test_index, zero_solvent, one_solvent, two_solvent, three_solvent,
         solvent_test):
    # train_loader = load_data(zero_solvent, "train", one_solvent, train_lable_index, two_solvent,
    #                          three_solvent)  #
    # know_solvent_test_loader = load_data(zero_solvent, "test", one_solvent, know_solvent_test_index, two_solvent,
    #                        three_solvent)  #
    # zero_solvent_test_loader = load_test_data("test", know_solvent_test_index, zero_solvent)
    # one_solvent_test_loader = load_test_data("test", know_solvent_test_index, one_solvent)
    # two_solvent_test_loader = load_test_data("test", know_solvent_test_index, two_solvent)
    # three_solvent_test_loader = load_test_data("test", know_solvent_test_index, three_solvent)
    unknow_solvent_test_loader = load_test_data("test", unknow_solvent_test_index, solvent_test)  #
    return "-", "-", "-", "-", unknow_solvent_test_loader
...
if __name__ == '__main__':
    ...
    data_list = []
    print("--------------------------build_model----------------------------")
    test_model = build_model(checkpoint_PATH=args.checkpoint_PATH)
    ...
    know_solvent_test_lable_index = test_lable_index  # 10000
    unknow_solvent_test_lable_index = train_lable_index + test_lable_index  # 90000

    know_solvent_test_batch = args.batchsize
    unknow_solvent_test_batch = args.batchsize * 9
    batch_num = int(len(unknow_solvent_test_lable_index) / unknow_solvent_test_batch)
    # i = 0
    print("batch_num=", batch_num)
    SUM_konw_solvent_zero5 = 0
    SUM_konw_solvent_one5 = 0
    SUM_konw_solvent_two5 = 0
    SUM_konw_solvent_three5 = 0

    SUM_konw_solvent_test = 0

    SUM_unkonw_solvent_test5 = 0
    SUM_unkonw_solvent_test10 = 0
    SUM_unkonw_solvent_test = 0

    true_label_data = 0
    pred_label_data = 0
    for current_iter in range(batch_num):

        print("current_epoch=", 0, "current_iter=", current_iter, "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        temp_know_solvent_test_index = know_solvent_test_lable_index[current_iter * know_solvent_test_batch:(current_iter + 1) * know_solvent_test_batch]
        temp_unknow_solvent_test_index = unknow_solvent_test_lable_index[current_iter * unknow_solvent_test_batch: (current_iter + 1) * unknow_solvent_test_batch]
        # test_list = temp_val_lable_index

        zero_solvent_test_loader, one_solvent_test_loader, two_solvent_test_loader, three_solvent_test_loader, unknow_solvent_test_loader = LOAD(temp_know_solvent_test_index, temp_unknow_solvent_test_index,
                                                     zero_solvent, one_solvent, two_solvent, three_solvent,
                                                     solvent_test)

        # SUMzero5, _, SUM_know_test = val_another_solvent(test_model,
        #                                                          zero_solvent_test_loader,
        #                                                          solute, zero_solvent)
        # SUMone5, _, SUM_know_test = val_another_solvent(test_model,
        #                                                          one_solvent_test_loader,
        #                                                          solute, one_solvent)
        # SUMtwo5, _, SUM_know_test = val_another_solvent(test_model,
        #                                                          two_solvent_test_loader,
        #                                                          solute, two_solvent)
        # SUMthree5, _, SUM_know_test = val_another_solvent(test_model,
        #                                                          three_solvent_test_loader,
        #                                                          solute, three_solvent)

        # SUM_konw_solvent_zero5 = SUM_konw_solvent_zero5 + SUMzero5
        # SUM_konw_solvent_one5 = SUM_konw_solvent_one5 + SUMone5
        # SUM_konw_solvent_two5 = SUM_konw_solvent_two5 + SUMtwo5
        # SUM_konw_solvent_three5 = SUM_konw_solvent_three5 + SUMthree5
        # SUM_konw_solvent_test = SUM_konw_solvent_test + SUM_know_test


        SUM_test5, SUM_test10, SUM_unknow_test, true_label_data, pred_label_data = val_another_solvent(test_model, unknow_solvent_test_loader, solute, solvent_test, true_label_data, pred_label_data, current_iter)
        SUM_unkonw_solvent_test5 = SUM_unkonw_solvent_test5 + SUM_test5
        SUM_unkonw_solvent_test10 = SUM_unkonw_solvent_test10 + SUM_test10
        SUM_unkonw_solvent_test = SUM_unkonw_solvent_test + SUM_unknow_test
        # if (current_iter %100 == 0):
        #     print("SUM_konw_solvent_zero5=", SUM_konw_solvent_zero5)
        #     print("SUM_konw_solvent_one5=", SUM_konw_solvent_one5)
        #     print("SUM_konw_solvent_two5=", SUM_konw_solvent_two5)
        #     print("SUM_konw_solvent_three5=", SUM_konw_solvent_three5)
        #     print("SUM_konw_solvent_test=", SUM_konw_solvent_test)
        #     print("SUM_unkonw_solvent_test5=", SUM_unkonw_solvent_test5)
        #     print("SUM_unkonw_solvent_test10=", SUM_unkonw_solvent_test10)
        #     print("SUM_unkonw_solvent_test=", SUM_unkonw_solvent_test)
        #     print("zeros = ", (SUM_konw_solvent_zero5 * 100) / SUM_konw_solvent_test, "%",
        #           "ones = ", (SUM_konw_solvent_one5 * 100) / SUM_konw_solvent_test, "%",
        #             "twos = ",(SUM_konw_solvent_two5 * 100) / SUM_konw_solvent_test, "%",
        #            "three = ",(SUM_konw_solvent_three5 * 100) / SUM_konw_solvent_test, "%",
        #            "unkonw5 = ",(SUM_unkonw_solvent_test5 * 100) / SUM_unkonw_solvent_test, "%",
        #           "unkonw10 = ",(SUM_unkonw_solvent_test10 * 100) / SUM_unkonw_solvent_test, "%")
    #
    # print("SUM_konw_solvent_zero5=", SUM_konw_solvent_zero5)
    # print("SUM_konw_solvent_one5=", SUM_konw_solvent_one5)
    # print("SUM_konw_solvent_two5=", SUM_konw_solvent_two5)
