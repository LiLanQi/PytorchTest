...
from MyFinalGNN6 import MyNewGNN, MyValModel
...
parser.add_argument('--root_path', type=str, default="./check_point/MyFinalGNNTest5/")
# parser.add_argument('--check_point_name', type=str, default="-MyFinalGNNTest5_net.pth.tar")
# parser.add_argument('--check_point_name_five', type=str, default="-MyFinalGNNTest5_net_in_5.pth.tar")
# parser.add_argument('--check_point_name_ten', type=str, default="-MyFinalGNNTest5_net_in_10.pth.tar")
parser.add_argument('--checkpoint_PATH', type=str,
                    default="./check_point/MyFinalGNNTest5/3/80-MyFinalGNNTest5_net.pth.tar")
...
writer = SummaryWriter('MyFinalGNNTest6')
...
class trainset(Dataset):
    ...
    def __init__(self, zero_solvent, phase, one_solvent, lable_index, two_solvent, three_solvent, solvent_test):
        ...
        path_list_four_solvent = readFileData(solvent_test["root_dir"], lable_index, solvent_test["solvent"])
        # count0= 4566 countN= 3486 countC= 13194 countF= 216 countS= 324 countH= 26274
        ...
        self.solute_list_four_solute, self.solvent_list_four_solvent, self.lables_list_four_solvent = self.__read_data__(
            path_list_four_solvent,
            lable_index, solvent_test["solvent"])
    ...
    def __getitem__(self, index):
        ...
        solute_data_four = self.solute_list_four_solute[index]
        solvent_data_four = self.solvent_list_four_solvent[index]
        lable_four = self.lables_list_four_solvent[index]

        return solute_data_zero, solvent_data_zero, lable_zero, solute_data_one, solvent_data_one, lable_one, solute_data_two, solvent_data_two, lable_two, solute_data_three, solvent_data_three, lable_three, solute_data_four, solvent_data_four, lable_four

    def __len__(self):
        return len(self.solute_list_zero_solute)


def load_data(zero_solvent, phase, one_solvent, lable_index, two_solvent, three_solvent, solvent_test):
    data = trainset(zero_solvent, phase, one_solvent, lable_index, two_solvent, three_solvent, solvent_test)
...
def val(net, val_loader, solute, zero_solvent, one_solvent, two_solvent, three_solvent, four_solvent):
    net.eval()
    SUMzero1 = 0
    SUMzero2 = 0
    SUMzero3 = 0
    ...
    for i_batch, batch_data in tqdm(enumerate(val_loader)):
        labels, solute_data_zero, solute_data_one, solute_data_two, solute_data_three, solute_data_four, solvent_data_zero, solvent_data_one, solvent_data_two, solvent_data_three, solvent_data_four = para_to_device(
            batch_data)
        with torch.set_grad_enabled(False):  #
            outputs = net(solute_data_zero, solvent_data_zero, solute, zero_solvent,
                          ...
                          solute_data_four, solvent_data_four, four_solvent).squeeze(-1)
        ...
        for i in range(deviation.shape[0]):
            if (i < args.batchsize):
                ...
                if (deviation[i] < 0.05):
                    SUMtwo5 = SUMtwo5 + 1
            elif (i >= 3 * args.batchsize and i < 4 * args.batchsize):
                if (deviation[i] < 0.01):
    ...
    four_accuracy = (SUMfour5) / (len(val_loader) * args.batchsize)
    ...
    writer.add_scalars('Accuracy',
                       {"zero_acc": zero_accuracy, "one_acc": one_accuracy, "two_acc": two_accuracy,
                        "three_acc": three_accuracy, "four_acc": four_accuracy},
                       current_iter + current_epoch * batch_num)
    schedular.step(metrics=three_accuracy)
    return zero_accuracy, one_accuracy, two_accuracy, three_accuracy, four_accuracy
...
def val_another_solvent(test_model, test_loader, solute, solvent_test, PATH1, bestSUM_solvent_in_5,
    ...
    if (SUMtest10 > bestSUM_solvent_in_10):
        ...
        best_PATH10 = PATH1 + args.check_point_name_ten
    if (SUMtest5 > bestSUM_solvent_in_5):
        bestSUM_solvent_in_5 = SUMtest5
        best_PATH5 = PATH1 + args.check_point_name_five
...
def para_to_device(batch_data):
    solute_data_zero, solvent_data_zero, label_zero, solute_data_one, solvent_data_one, label_one, solute_data_two, solvent_data_two, label_two, solute_data_three, solvent_data_three, label_three, solute_data_four, solvent_data_four, label_four = batch_data
    ...
    solute_data_four = solute_data_four.to(DEVICE)
    ...
    solvent_data_four = solvent_data_four.to(DEVICE)
    return labels, solute_data_zero, solute_data_one, solute_data_two, solute_data_three, solute_data_four, solvent_data_zero, solvent_data_one, solvent_data_two, solvent_data_three, solvent_data_four
...
def train(net, optimizer, criterion, test_model, train_loader, solute, zero_solvent, one_solvent, two_solvent,
          ...
          bestSUM_solvent_in_10, current_iter, current_epoch, batch_num, schedular, solute_data, solvent_data,
          solution_data, label_data, MLP_data):
    loss_list = []
    # tf_flag = 0
    net.train()
    total_loss = 0
    for i_batch, batch_data in tqdm(enumerate(train_loader)):
        ...
        with torch.set_grad_enabled(True):  #
            solute, solvent, solution, data = net(solute_data_zero, solvent_data_zero, solute, zero_solvent,
                                                  solute_data_one, solvent_data_one, one_solvent,
                                                  solute_data_two, solvent_data_two, two_solvent,
                                                  solute_data_three, solvent_data_three, three_solvent,
                                                  solute_data_four, solvent_data_four, solvent_test)
            print(type(solution))
            solution = solution.cpu().detach()
            solution = solution.numpy()
            solute = solute.cpu().detach().numpy()
            solvent = solvent.cpu().detach().numpy()
            labels = labels.cpu().detach().numpy().reshape(-1, 1)
            data = solution.cpu().detach().numpy()
            if (current_iter == 0):
                solute_data = solute
                ...
                solute_data = numpy.vstack((solute_data, solute))
                solvent_data = numpy.vstack((solvent_data, solvent))
                solution_data = numpy.vstack((solution_data, solution))
                label_data = numpy.vstack((label_data, labels))
                MLP_data = numpy.vstack((MLP_data, data))

            np.savetxt('../solute_data.csv', solute_data, delimiter=',')
            np.savetxt('../solvent_data.csv', solvent_data, delimiter=',')
            np.savetxt('../solution_data.csv', solution_data, delimiter=',')
            np.savetxt('../label_data.csv', label_data, delimiter=',')
            np.savetxt('../MLP_data.csv', MLP_data, delimiter=',')
            # print("solution=", solution)
            # file_csv = codecs.open("../set2set.csv", 'w+')
    ...
    # writer.add_scalar('current_lr', optimizer.state_dict()['param_groups'][0]['lr'], current_iter+current_epoch*batch_num)
    # writer.add_scalar('total loss', total_loss / len(train_loader), current_iter+current_epoch*batch_num)
    # loss_list.append(total_loss / len(train_loader))
    # PATH1 = args.root_path + str(current_epoch) + "/" +str(current_iter)
    ...
    #            args.root_path + str(current_epoch) + "/" + str(current_iter) + args.check_point_name)
    # zero_accuracy, one_accuracy, two_accuracy, three_accuracy, four_accuracy = val(net, val_loader, solute, zero_solvent, one_solvent, two_solvent, three_solvent, solvent_test)
    #
    #
    #
    # # test_model.load_state_dict(net.state_dict())
    # # best_PATH5, best_PATH10, bestSUM_solvent_in_5, bestSUM_solvent_in_10 = val_another_solvent(test_model, test_loader, solute, solvent_test,
    # #                                                            PATH1, bestSUM_solvent_in_5, bestSUM_solvent_in_10, best_PATH5,
    # #                                                            best_PATH10, current_iter, zero_accuracy, one_accuracy, two_accuracy, three_accuracy, current_epoch, batch_num, schedular)
    # #
    ...
    return best_PATH5, best_PATH10, bestSUM_solvent_in_5, bestSUM_solvent_in_10, solute_data, solvent_data, solution_data, label_data, MLP_data
...
def TRAIN(net, optimizer, criterion, test_model, train_loader, solute, zero_solvent, one_solvent, two_solvent,
          ...
          bestSUM_solvent_in_10, current_iter, current_epoch, batch_num, schedular, solute_data, solvent_data,
          solution_data, label_data, MLP_data):
    best_PATH5, best_PATH10, bestSUM_solvent_in_5, bestSUM_solvent_in_10, solute_data, solvent_data, solution_data, label_data, MLP_data = train(
        net, optimizer, criterion, test_model,
        ...
        current_epoch, batch_num, schedular, solute_data, solvent_data, solution_data, label_data, MLP_data)
    return best_PATH5, best_PATH10, bestSUM_solvent_in_5, bestSUM_solvent_in_10, solute_data, solvent_data, solution_data, label_data, MLP_data
...
if __name__ == '__main__':
    ...
    data_list = []
    print("--------------------------build_model----------------------------")
    net, optimizer, criterion, test_model, schedular = build_model(checkpoint_PATH=args.checkpoint_PATH)
    ...
    for current_epoch in range(1):
        epoch_root_path = args.root_path + str(current_epoch)
        if (os.path.exists(args.root_path) == False):
            os.mkdir(args.root_path)
        if (os.path.exists(epoch_root_path) == False):
            os.mkdir(epoch_root_path)
        best_PATH5 = ""
        best_PATH10 = ""
        ...
        label_data = 0
        MLP_data = 0
        for current_iter in range(0, 12):
            print("current_epoch=", current_epoch, "current_iter=", current_iter,
                  "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            temp_train_lable_index = train_lable_index[current_iter * val_batch:(current_iter + 1) * val_batch]
            temp_val_lable_index = val_lable_index[current_iter * val_batch: int(current_iter + 1) * val_batch]
            test_list = temp_val_lable_index

            train_loader, val_loader = LOAD(temp_train_lable_index, temp_val_lable_index, test_list,
            ...
            best_PATH5, best_PATH10, bestSUM_solvent_in_5, bestSUM_solvent_in_10, solute_data, solvent_data, solution_data, label_data, MLP_data = TRAIN(
                ...
                batch_num, schedular, solute_data, solvent_data, solution_data, label_data, MLP_data)