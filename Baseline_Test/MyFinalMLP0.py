...
class MyNewGNN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout, DEVICE, batch_size):
        ...
        self.solute_mlp = MLP([nclass, nclass, nclass])

        self.solvent_mlp = MLP([nclass, nclass, nclass])
    ...
    def forward(self, solute_data_zero, solvent_data_zero, solute, zero_solvent,
        ...
        solute_data_zero = self.solute_mlp(init_solute_zero) + init_solute_zero
        solute_data_one = self.solute_mlp(init_solute_one) + init_solute_one
        solute_data_two = self.solute_mlp(init_solute_two) + init_solute_two
        solute_data_three = self.solute_mlp(init_solute_three) + init_solute_three
        ...
        solvent_data_zero = self.solvent_mlp(init_solvent_zero) + init_solvent_zero
        solvent_data_one = self.solvent_mlp(init_solvent_one) + init_solvent_one
        solvent_data_two = self.solvent_mlp(init_solvent_two) + init_solvent_two
        solvent_data_three = self.solvent_mlp(init_solvent_three) + init_solvent_three
...
class MyValModel(nn.Module):
    ...
    def forward(self, solute, solute_data_test, solvent_data_test, solvent_test):
        ...
        # 溶质进行2层GraphSAGE+resnet
        solute_data_test = self.solute_mlp(init_solute_test) + init_solute_test

        solvent_data_test = solvent_data_test.reshape(-1, solvent_data_test.shape[-1])

        init_solvent_test = self.fc1(solvent_data_test)
        # 溶剂进行2层GraphSAGE+resnet
        solvent_data_test = self.solute_mlp(init_solvent_test) + init_solvent_test
