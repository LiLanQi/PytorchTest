...
class test_trainset(Dataset):
    ...
    def __read_data__(self, path_list, lable_index, solvent):
        ...
        #
        atom_to_num={"H":1, "C":6, "N":7, "O":8, "F":9, "S":16}
        current_data = -1
        for path in path_list:
            ...
            for i in range(5, 2081):
                ...
                position_list.append(atom_to_num[single_temp_atom])
            ...
            # temp = torch.stack(torch.tensor(temp_result_list))
            temp = torch.tensor(position_list).reshape(-1, 3)
...
class trainset(Dataset):
    ...
    def __read_data__(self, path_list, lable_index, solvent):
        ...
        for path in path_list:
            ...
            temp = torch.tensor(position_list).reshape(-1, 4)
            # temp = torch.cat((temp, speed), 1)
            solute_data = temp[:173 * 12, :]
            solvent_data = temp[173 * 12:, :]
            solute_list.append(solute_data)  #
...
def val_another_solvent(test_model, test_loader, solute, solvent_test, PATH1, bestSUM_solvent_in_5, bestSUM_solvent_in_10, best_PATH5,
    ...
    if (SUMtest10 > bestSUM_solvent_in_10):
        ...
        best_PATH10 = PATH1 + "-MyDimeTest0_net_in_10.pth.tar"
    if (SUMtest5 > bestSUM_solvent_in_5):
        bestSUM_solvent_in_5 = SUMtest5
        best_PATH5 = PATH1 + "-MyDimeTest0_net_in_5.pth.tar"
...
def build_model(checkpoint_PATH):
    with torch.no_grad():
        net = DimeNet(hidden_channels=128, out_channels=1, num_blocks=6,
...
def train(net, optimizer, criterion, test_model, train_loader, solute, zero_solvent, one_solvent, two_solvent,
    ...
    for i_batch, batch_data in tqdm(enumerate(train_loader)):
        ...
        data_batch = get_graph_pool_batch(len_solute, len_solvent_zero, len_solvent_one, len_solvent_two, len_solvent_three).to(DEVICE)
        solution_zero = torch.cat((solute_data_zero, solvent_data_zero), 1).reshape(-1, 4)
        solution_one = torch.cat((solute_data_one, solvent_data_one), 1).reshape(-1, 4)
        solution_two = torch.cat((solute_data_two, solvent_data_two), 1).reshape(-1, 4)
        solution_three = torch.cat((solute_data_three, solvent_data_three), 1).reshape(-1, 4)
        ...
        with torch.set_grad_enabled(True):  #
            ...
            # print("data[:,0].type=", type(data[:, 0].long()))
            # print("data[:,0].type=", data[:, 0].long().dtype)
            outputs = net(data[:,0].long(), data[:,1:], data_batch).squeeze(-1)
    ...
    PATH1 = "./check_point/MyDimeTest0/" + str(current_epoch) + "/" +str(current_iter)
    ...
    if(current_iter % 20 ==0):
        ...
        torch.save({'state_dict': net.state_dict(), 'optimizer': optimizer.state_dict()},
               "./check_point/MyDimeTest0/" + str(current_epoch) + "/" + str(current_iter) + "-MyDimeTest0_net.pth.tar")
...
if __name__ == '__main__':
    ...
    for current_epoch in range(args.epoch):
        root_path = "./check_point/MyDimeTest0/" + str(current_epoch)
