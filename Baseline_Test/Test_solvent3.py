...
parser.add_argument('--checkpoint_PATH', type=str, default="./check_point/MyFinalGNNTest0/3/35-MyFinalGNNTest0_net_in_5.pth.tar")
...
def val_another_solvent(test_model, test_loader, solute, solvent_test, true_label_data, pred_label_data, current_iter):
    ...
    for i_batch, batch_data in tqdm(enumerate(test_loader)):
        ...
        if (solvent_test["solvent"] == "ACE"):
            ...
            np.savetxt('../ACE_true_label_data.csv', true_label_data, delimiter=',')
            np.savetxt('../ACE_pred_label_data.csv', pred_label_data, delimiter=',')
        for i in range(deviation.shape[0]):
            if (i < args.batchsize):
                if (deviation[i] < 0.01):
                    SUMtest1 = SUMtest1 + 1
