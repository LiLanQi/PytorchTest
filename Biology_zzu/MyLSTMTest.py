...
writer = SummaryWriter('MyLSTMTest1')
...
class trainset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        output_data = self.data[index + 100,:]
...
if __name__ == '__main__':
    ...
    rnn = MyLSTM(input_size=3, hidden_size=250, num_layers=2)
    rnn = rnn.to(DEVICE)
    # rnn.apply(weight_init)
    rnn.train()
    optimizer = torch.optim.Adam(rnn.parameters(), lr=args.lr, betas=(0.9, 0.99))
    criterion = torch.nn.MSELoss()
    h0 = Variable(torch.zeros(2, args.batchsize, 250))
    c0 = Variable(torch.zeros(2, args.batchsize, 250))
    h0 = h0.to(DEVICE)
    c0 = c0.to(DEVICE)
    for epoch in range(10):
        ...
        for i_batch, batch_data in tqdm(enumerate(train_loader)):
            ...
            label_data = label_data.unsqueeze(0).to(DEVICE)

            optimizer.zero_grad()
            outputs = rnn(input_data, h0, c0)
            ...
            # if(i_batch % 10 == 9):
                ...
                # print("outputs-output_data=", abs(outputs - label_data))
                # print("deviation=", abs(outputs - label_data) / abs(label_data))
                # print("loss=", loss.item())
                # print("epoch=",epoch)
        writer.add_scalar('train loss', total_loss / len(train_loader), epoch)
    print("len(train_loader)=",len(train_loader)) #225
    PATH1 = './MyLSTMest1_net.pth'
    ...
    test_data = test_all_data[0:100,:].unsqueeze(0)
    label_data = test_all_data[100:,:]
    print("label_data=", label_data)
    total_loss = 0
    prediction_data = torch.zeros(1,1,3)
    for i_batch in range(10000,12000):
        test_data = test_data.to(DEVICE)
        if(i_batch != 10000):
            test_data = torch.cat((test_data[:,1:,:],prediction_data), 1)
            print("test_data=",test_data)
        test_correct_label = label_data[i_batch-10000,:].unsqueeze(0).to(DEVICE)
        with torch.set_grad_enabled(False):
            outputs = rnn(test_data, h0, c0)
            loss = criterion(outputs, test_correct_label)
        prediction_data = outputs
        prediction_data = prediction_data.to(DEVICE)
        deviation = (abs(outputs - test_correct_label) / abs(test_correct_label)).squeeze(0).squeeze(0)
        print("outputs=", outputs)
        ...
        for i in range(deviation.shape[0]):
            ...
            if (deviation[i] < 0.05):
                SUM5 = SUM5 + 1
        writer.add_scalar('predic loss', loss.item(), i_batch-10000)
    print("SUM1 = ", SUM1, "SUM2 = ", SUM2, "SUM3 = ", SUM3, "SUM4 = ", SUM4, "SUM5 = ", SUM5)
    print("共有", SUM5, "个坐标数据误差在5%以内")
