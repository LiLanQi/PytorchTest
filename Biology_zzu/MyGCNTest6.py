...
writer = SummaryWriter('MyGCNTest6')
...
if __name__ == '__main__':
    ...
    print('MyGCNTest6 Finished Training, lr=', args.lr, "batchsize=", args.batchsize)
    PATH1 = './MyGCNTest6_net.pth'
    ...
    for i_batch, batch_data in tqdm(enumerate(test_loader)):
        ...
        for i in range(deviation.shape[0]):
            if (deviation[i] < 0.01):
                SUM1 = SUM1 + 1
            elif (deviation[i] < 0.02):
                SUM2 = SUM2 + 1
            elif (deviation[i] < 0.03):
                SUM3 = SUM3 + 1
            elif (deviation[i] < 0.04):
                SUM4 = SUM4 + 1
            elif (deviation[i] < 0.05):
                SUM5 = SUM5 + 1
