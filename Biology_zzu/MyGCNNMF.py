...
class MyGCNNMF(nn.Module):
    #(nfeat=3,nhid=8,nclass=16,dropout=0.5)
    def __init__(self, nfeat, nhid, nclass, dropout):
        super(MyGCNNMF, self).__init__()

        self.gc1 = GraphConvolution(nfeat, nhid)
        self.gc2 = GraphConvolution(nhid, nclass)
        #唯一需要改的是这一行，如果溶剂为NMF，则为14226 * 16，如果溶剂为ACE，则为11016 * 16，其中乘号前面的数字是由溶质和溶剂拼接而成
        self.fc1 = nn.Linear(14226 * 16, 360)
