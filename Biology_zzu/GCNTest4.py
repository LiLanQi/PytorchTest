...
import argparse
import os
from MySoluteGCN import MySoluteGCN
from MyReadoutNet import MyReadoutNet
...
class trainset(Dataset):
    def __init__(self, rootdir, result_src, phase, lable_index):
        ...
        for path in path_list:
            ...
            # temp = torch.cat((torch.unsqueeze(temp[:, 0], dim=1), (temp[:, 1:] - Min) / (Max - Min)), 1)
...
if __name__ == '__main__':
    ...
    # print("train_lable_index=",train_lable_index)
    # print("test_lable_index=", test_lable_index)
    ...
    # print("type(solute_adj)=",type(solute_adj))
    # print("solute_adj=",solute_adj)
    # print("solute_adj.shape=",solute_adj.shape)
    # print("dict_solvent=",dict_solvent)
    # print("solvent_adj.shape=", solvent_adj.shape)
    train_loader = load_train(args.data_src, args.batchsize, args.result_src, "train",train_lable_index)
    net = MySoluteGCN(nfeat=3,
                nhid=8,
                nclass=16,
    ...
    for epoch in range(100):
        ...
        for i_batch, batch_data in tqdm(enumerate(train_loader)):
            ...
            with torch.set_grad_enabled(True):  # 当requires_grad设置为False时,反向传播时就不会自动求导了，因此大大节约了显存或者说内存。
                outputs_solute = net(solute_data,solute_adj)
                outputs_solvent = net(solvent_data, solvent_adj)
            # print("outputs_solute=", outputs_solute)
            # print("outputs_solute.shape=", outputs_solute.shape)#(4,2076,16)
            # print("outputs_solvent=", outputs_solvent)
            # print("outputs_solvent.shape=", outputs_solvent.shape)#(4,8940,16)

            data = torch.cat((outputs_solute, outputs_solvent), 1)#(4,11016,16)
            data = data.to(DEVICE)
    ...
    print('GCNTest4 Finished Training, lr=', args.lr, "batchsize=", args.batchsize)
    PATH1 = './GCNTest4_net.pth'
    PATH2 = './GCNTest4_model.pth'
