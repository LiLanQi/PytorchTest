...
writer = SummaryWriter('MyGCNTestInNMF')
...
def get_solvent_position(rootdir):
    ...
    for path in path_list:
        file = open(path, "r")
        file_list = file.readlines()
        for i in range(len(file_list) - 11, len(file_list) - 2):
            temp_data = file_list[i].strip().split('1.00  0.00')[0][26:].strip()
            temp_data = temp_data.split('.')
            temp_atom = file_list[i][13:16].strip()  # 该点所对应的原子
            dict_solvent[temp_atom] = i-(len(file_list) - 11) #这里应该也是11
        break
    return dict_solvent
...
#9是指溶剂原子数，1350是指一个时刻有1350张溶剂图,1350*9=12150
def get_solvent_adj():
    solvent_adj = torch.zeros(9*1350,9*1350)
    bonds = 'C1-O1', 'C1-H1', 'C1-N1', 'N1-H2', 'C2-N1', 'C2-H3', 'C2-H4', 'C2-H5'
    for connect_atom in bonds:
        left_atom = connect_atom.split("-")[0]
        right_atom = connect_atom.split("-")[-1]
        for i in range(1350):
            left_atom_value = dict_solvent[left_atom] + 9 * i
            right_atom_value = dict_solvent[right_atom] + 9 * i
            solvent_adj[left_atom_value][right_atom_value] = 1
            solvent_adj[right_atom_value][left_atom_value] = 1
    solvent_adj =  normalize(solvent_adj + torch.eye(9*1350,9*1350))
    return solvent_adj
...
class trainset(Dataset):
    def __init__(self, rootdir, result_src, phase, lable_index):
        ...
        for path in path_list:
            ...
            solute_data = temp[:173*12, :]#173*12=2076
            solvent_data = temp[173*12:, :]
            solute_list.append(solute_data)  # 存入每个NMF文件的溶质图
            solvent_list.append(solvent_data)# 存入每个NMF文件的溶剂图
        for i in range(len(lable_index)):
            lables_list.append(total_labels[lable_index[i]])# 存入每个NMF文件的标签
        self.solute_list = solute_list
        self.solvent_list = solvent_list
        self.lables_list = lables_list  # 存入所有NMF_LJ文件对应的output值
...
if __name__ == '__main__':
    ...
    print('MyGCNTestInNMF Finished Training, lr=', args.lr, "batchsize=", args.batchsize)
    PATH1 = './MyGCNTestInNMF_net.pth'
